...
async def list_voices():
    """Lista as vozes disponíveis na biblioteca edge_tts."""
    voices = await edge_tts.list_voices()
    return voices

async def generate_speech(text, voice, output_file, rate, pitch):
    """Gera o arquivo de áudio a partir do texto usando a voz especificada."""
    communicate = edge_tts.Communicate(text, voice, rate=rate, pitch=pitch)
    await communicate.save(output_file)
    return output_file

def get_languages_and_voices(voices):
    """Organiza as vozes por idioma e retorna a lista de idiomas e o mapeamento."""
        ...
        if not text:
            ...
            with st.spinner("Gerando áudio..."):
                try:
                    ...
                    # Carregar o áudio com st.audio
                    if os.path.exists(audio_file):
                        audio_value = open(audio_file, "rb")
                        st.audio(audio_value, format="audio/mp3")
                        audio_value.close()
                        
                        # Usar JavaScript para tocar automaticamente e CSS para ocultar o widget
                        st.markdown(
                            """
                            <style>
                                /* Ocultar o widget de áudio do Streamlit */
                                audio {
                                    display: none;
                                }
                            </style>
                            <script>
                                // Encontrar o elemento de áudio gerado pelo st.audio
